...
TriMat::TriMat(int matsize)
{
    ...
    if (matsize > ROWLIMIT)
    {
        std::cerr << "The matrix cannot exceed size " << ROWLIMIT <<
                "x " << ROWLIMIT << std::endl;
        exit(1);
    }
...
void TriMat::PutElement (double item, int i, int j)
{
    ...
    if ((i < 0 || i >= n) || (j < 0 || j >= n))
    {
        std::cerr << "PutElement: index out of range 0 to "
             << (n-1) << std::endl;
        exit (1);
    }
...
void TriMat::ReadMat(void)
{
    ...
    for (i = 0; i < n; i++)			// scan rows
        ...
        {
            std::cin >> item;			// Read matrix element[i,j]
...
void  TriMat::WriteMat(void) const
{
    ...
    {
        for  (j = 0; j < n; j++)
            std::cout << std::setw(7) << GetElement(i,j);
        std::cout << std::endl;
    }
}
