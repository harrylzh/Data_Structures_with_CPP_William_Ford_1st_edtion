...
struct TellerStats
{
    ...
    int totalService;       // total time servistd::cing customers
};
...
Simulation::Simulation(void)
{
    ...
    std::cout << "Enter the simulation time in minutes: ";
    std::cin >> simulationLength;
    std::cout << "Enter the number of bank tellers: ";
    std::cin >> numTellers;
    std::cout << "Enter the range of arrival times in minutes: ";
    std::cin >> arrivalLow >> arrivalHigh;
    std::cout << "Enter the range of service times in minutes: ";
...
void Simulation::RunSimulation(void)
{
    ...
    while (!pq.PQEmpty())
    {
        ...
        if (e.GetEventType() == arrival)
        {
            ...
            std::cout << "Time: " << std::setw(2) << e.GetTime()
                 << "  " << "arrival of customer "
                 << e.GetCustomerID() << std::endl;
...
void Simulation::PrintSimulationResults(void)
{
    ...
    std::cout << "Simulation of " << simulationLength
         << " minutes" << std::endl;
    std::cout << "   No. of Customers:  " << cumCustomers << std::endl;
    std::cout << "   Average Customer Wait: ";
    
    avgCustWait = float(cumWait)/cumCustomers + 0.5;
    std::cout << avgCustWait << " minutes" << std::endl;
    for(i=1;i <= numTellers;i++)
    {
        std::cout << "    Teller #" << i << "  % Working: ";
        ...
        std::cout << tellerWorkPercent << std::endl;
    }
}
